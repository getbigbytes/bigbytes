...
<AccordionGroup>
  <Accordion title="Docker" icon="docker">
    ...
    <CodeGroup>
      ```bash Mac/Linux
      docker run -it -p 6789:6789 -v $(pwd):/home/src digitranslab/bigbytes /app/run_app.sh bigbytes start [project_name]
      ```
      ```bash Windows Command Line
      docker run -it -p 6789:6789 -v "%cd%:/home/src" digitranslab/bigbytes /app/run_app.sh bigbytes start [project_name]
      ```
      ...
      sudo docker run -it -p 6789:6789 -v $(pwd):/home/src digitranslab/bigbytes /app/run_app.sh bigbytes start [project_name]
      ```
    </CodeGroup>
  ...
  <Accordion title="Docker Compose" icon="Docker">
    ...
    <CodeGroup>
      ```bash Mac/Linux
      git clone https://github.com/digitranslab/compose-quickstart.git bigbytes-quickstart \
    ...
    We're cloning our Docker Compose [quickstart repo](https://github.com/digitranslab/compose-quickstart.git), copying `dev.env` to `.env`, which is ignored by git in our configuration (to avoid exposing secrets), then running `docker compose up` to start a Bigbytes server.
   ...
   <Accordion title="Kubernetes" icon="network-wired">
      <Steps>
        ...
        <Step title="Download and update Kubernetes config file">
          First, download the Bigbytes Kubernetes config file [here](https://github.com/digitranslab/bigbytes/blob/master/kube/app.yaml).
...
<p align="center">
<img src="https://github.com/digitranslab/assets/blob/main/quickstart/open-pipeline.gif?raw=True"/>
</p>
...
<Frame>
<img src="https://github.com/digitranslab/assets/blob/main/terminal.png?raw=True"/>
</Frame>
