...
Here is a [sample project](https://github.com/digitranslab/demo%5Fetl%5Fpipeline) and a sample folder structure:
...
Project-level data is stored in the `bigbytes_data` directory— located at the same level as your project folder.

`bigbytes_data` holds project-level cache data and stores the result of Block runs, which are then returned to the user via the UI. Here are the locations for various components of `bigbytes_data`:

- SQLite DB: `bigbytes_data/[project_folder]/bigbytes.db`
  ...
  - You can connect to the SQLite DB like any other database. The JDBC URL `jdbc:sqlite:PATH`, where PATH is `/home/src/bigbytes_data/[project_folder]/bigbytes.db` for Docker installs or `~/.bigbytes_data/[project_folder]/bigbytes.db` for pip installs.
- Block output: `bigbytes_data/[project_folder]/pipelines/[pipeline_uuid]/.variables/`
- Pipeline and block logs: `bigbytes_data/[project_folder]/pipelines/[pipeline_uuid]/.logs/`
- Cache: `bigbytes_data/[project_folder]/.cache`
