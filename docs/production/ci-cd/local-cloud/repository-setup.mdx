...
3.  Once you’re done developing, copy the contents of the this
    [Dockerfile template](https://github.com/digitranslab/docker/blob/master/Dockerfile)
    ...
    ENV PYTHONPATH="${PYTHONPATH}:/home/bigbytes_code"
...
4.  Your current folder structure should look like this:
    ...
    |   Dockerfile
    ```
5.  Build a custom Docker image using `bigbytes/bigbytes:latest` as the base and
...
6.  To test the new image works, run the following command:
    ```bash
    docker run -it -p 6789:6789 mageprod:latest /app/run_app.sh bigbytes start demo_project
    ```
