...
* To further customize the container config of the kubernetes executor, you can specify the `container_config` or `job` in the k8s executor config. Here is the example:
  ...
  k8s_executor_config:
    container_config:
      image: digitranslab/bigbytes:0.9.7
      env:
      - name: USER_CODE_PATH
...
**NB:** When deploying Bigbytes in a multi-container pod, you need to specify the environment variable `MAGE_CONTAINER_NAME`. If this variable is not set, Bigbytes will default to using the first container in the pod. To specify the Bigbytes container, you can use:
