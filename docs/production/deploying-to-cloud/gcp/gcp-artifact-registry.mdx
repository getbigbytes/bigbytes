...
5.  Tag the pulled Bigbytes docker image or use a previously tagged Docker image you
    ...
    - Sample commands if using vanilla Bigbytes Docker image:
        ...
        ```bash
        docker tag digitranslab/bigbytes:latest [region]-docker.pkg.dev/[project_id]/[repository]/bigbytes:latest
        ```

        - An example command could look like this:

            ```bash
            docker tag digitranslab/bigbytes:latest us-west2-docker.pkg.dev/materia-284023/mage-docker/bigbytes:latest
            ```
    ...
    - Sample commands if using previously tagged a custom Docker image using the tag `mageprod:latest`:
        ...
        - An example command could look like this if you previously tagged your custom Docker image with the tag `mageprod:latest`:
            ...
            ```bash
            docker tag mageprod:latest us-west2-docker.pkg.dev/materia-284023/mage-docker/bigbytes:latest
            ```
...
6.  Push the local Docker image to GCP Artifact Registry:
    ...
    - An example command could look like this:
        ...
        ```bash
        docker push us-west2-docker.pkg.dev/materia-284023/mage-docker/bigbytes:latest
        ```
