...
5.  Tag the pulled Bigbytes docker image or use a previously tagged Docker image you
    ...
    - Sample commands if using vanilla Bigbytes Docker image:
        ...
        ```bash
        docker tag bigbytes/bigbytes:latest [region]-docker.pkg.dev/[project_id]/[repository]/bigbytes:latest
        ```

        - An example command could look like this:

            ```bash
            docker tag bigbytes/bigbytes:latest us-west2-docker.pkg.dev/materia-284023/bigbytes-docker/bigbytes:latest
            ```
    ...
    - Sample commands if using previously tagged a custom Docker image using the tag `mageprod:latest`:
        ...
        - An example command could look like this if you previously tagged your custom Docker image with the tag `mageprod:latest`:
            ...
            ```bash
            docker tag mageprod:latest us-west2-docker.pkg.dev/materia-284023/bigbytes-docker/bigbytes:latest
            ```
