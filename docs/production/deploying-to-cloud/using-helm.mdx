...
Bigbytes helm repo: https://digitranslab.github.io/helm-charts/

### Add Helm repository
```bash
helm repo add bigbytes https://digitranslab.github.io/helm-charts
```

### Install Helm charts with default config
```
helm install my-bigbytes digitranslab/bigbytes
```
...
- Get the `values.yaml` with the command
    ...
    ```bash
    helm show values digitranslab/bigbytes > values.yaml
    ```
    
- Customize the mount volume
  - **Local**: Edit the `volumes` config in `values.yaml` to mount to your Bigbytes project path
  - **AWS EKS**: Provision the Elastic File System and create the [PersistentVolumeClaim](https://github.com/digitranslab/bigbytes-terraform-templates/blob/master/aws-eks/kube/efs-pvc.yaml) (using ReadWriteMany access mode). Then edit the `volumes` config to
    ```
    volumes:
      - name: bigbytes-fs
...
- Install the Helm charts with updated `values.yaml`
    ```bash
    helm install --values path/to/values.yaml my-bigbytes digitranslab/bigbytes
    ```
    
## Releases

https://github.com/digitranslab/helm-charts/releases
