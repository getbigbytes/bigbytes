...
Bigbytes helm repo: https://bigbytes.github.io/helm-charts/
...
```
helm install my-bigbytes bigbytes/bigbytes
```
...
- Get the `values.yaml` with the command
    ...
    ```bash
    helm show values bigbytes/bigbytes > values.yaml
    ```
...
- Install the Helm charts with updated `values.yaml`
    ```bash
    helm install --values path/to/values.yaml my-bigbytes bigbytes/bigbytes
    ```
