...
```bash
docker pull bigbytes/bigbytes:latest
```
...
docker run -it -p 6789:6789 -v $(pwd):/home/src \
  bigbytes/bigbytes /app/run_app.sh bigbytes init demo_project
...
In the `dags/` folder, create a new file named `create_bigbytes_pipelines.py`.
...
```bash
docker run -it -p 6789:6789 -v $(pwd):/home/src \
  bigbytes/bigbytes /app/run_app.sh bigbytes start demo_project
...
2. If you named your pipeline `etl demo` based on the tutorial from the previous
   step, then find a DAG named `bigbytes_pipeline_etl_demo`. If you named it
   something else, find a DAG with the prefix `bigbytes_pipeline_`.
3. Click on the DAG to view the detail page. The URL could typically be this:
   [http://localhost:8080/admin/airflow/tree?dag_id=bigbytes_pipeline_etl_demo](http://localhost:8080/admin/airflow/tree?dag_id=bigbytes_pipeline_etl_demo).
...
<img alt="Bigbytes in Airflow" src="https://github.com/digitranslab/assets/blob/main/bigbytes-airflow.jpg?raw=True" />
</Step>
</Steps>
