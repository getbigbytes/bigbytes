...
* Download the [Dockerfile template](https://github.com/digitranslab/bigbytes/blob/master/integrations/spark/Dockerfile) for Bigbytes with Spark environment.
* Change the current directory to the directory that contains the `Dockerfile`. Build the docker image with the command `docker build -t bigbytes_spark .`
...
### 1. Build Bigbytes docker image with Spark environment
* Download the [Dockerfile template](https://github.com/digitranslab/bigbytes/blob/master/integrations/spark/Dockerfile) for Bigbytes with Spark environment.
* Build the docker image with the command `docker build -t bigbytes_spark .`
...
* Download the [Dockerfile template](https://github.com/digitranslab/bigbytes/blob/master/integrations/hadoop/Dockerfile) for Bigbytes with Hadoop environment.
* Build the docker image with the command `docker build -t bigbytes_hadoop .`
...
these steps, please join our [Slack](https://www.bigbytes.io/chat)
...
```bash
docker run -it -p 6789:6789 -v $(pwd):/home/src bigbytes/bigbytes \
  /app/run_app.sh bigbytes start demo_project
```
...
def data_from_internet():
    url = 'https://raw.githubusercontent.com/digitranslab/datasets/master/restaurant_user_transactions.csv'
