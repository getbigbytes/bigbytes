...
from bigbytes.api.resources.PipelineResource import PipelineResource
from bigbytes.api.resources.ProjectResource import ProjectResource
from bigbytes.api.utils import authenticate_client_and_token
...
from bigbytes.server.active_kernel import (
    get_active_kernel_client,
    get_active_kernel_name,
    switch_active_kernel,
)
from bigbytes.server.execution_manager import (
...
from bigbytes.server.kernels import DEFAULT_KERNEL_NAME, KernelName
from bigbytes.server.logger import Logger
from bigbytes.server.utils.output_display import (
    add_execution_code,
    add_internal_output_info,
    get_block_output_process_code,
    get_pipeline_execution_code,
)
from bigbytes.services.spark.constants import ComputeServiceUUID
...
from bigbytes.shared.constants import ENV_DEV
from bigbytes.shared.hash import merge_dict
from bigbytes.shared.security import filter_out_env_var_values
from bigbytes.utils.code import reload_all_repo_modules
...
"""
    ...
    def format_error(cls, error: List[str], block_uuid: str = None) -> List[str]:
        ...
        useful to the user because they will contain internal Bigbytes method calls. The point
        of this method is to remove those unnecessary lines and replace hard-to-read font with
        more visible font.

        The `initial_regex` pattern is used to find the start of the Bigbytes method calls within the
        ...
        The `end_regex` pattern is used to find the end of the Bigbytes method calls. If a block UUID
