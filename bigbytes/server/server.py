...
from bigbytes.authentication.passwords import create_bcrypt_hash, generate_salt
from bigbytes.cache.block import BlockCache
from bigbytes.cache.block_action_object import BlockActionObjectCache
from bigbytes.cache.dbt.cache import DBTCache
from bigbytes.cache.file import FileCache
from bigbytes.cache.pipeline import PipelineCache
from bigbytes.cache.tag import TagCache
from bigbytes.cluster_manager.constants import ClusterType
from bigbytes.cluster_manager.manage import check_auto_termination
...
from bigbytes.data_preparation.shared.constants import MANAGE_ENV_VAR
from bigbytes.orchestration.constants import Entity
from bigbytes.orchestration.db import db_connection, safe_db_query, set_db_schema
from bigbytes.orchestration.db.database_manager import database_manager
from bigbytes.orchestration.db.models.oauth import Oauth2Application, Role, User
from bigbytes.orchestration.utils.distributed_lock import DistributedLock
from bigbytes.server.api.base import BaseHandler
from bigbytes.server.api.blocks import ApiPipelineBlockAnalysisHandler
from bigbytes.server.api.downloads import ApiDownloadHandler, ApiResourceDownloadHandler
from bigbytes.server.api.events import (
    ApiEventHandler,
    ApiEventMatcherDetailHandler,
    ApiEventMatcherListHandler,
)
from bigbytes.server.api.runs import ApiRunHandler
from bigbytes.server.api.triggers import ApiTriggerPipelineHandler
from bigbytes.server.api.v1 import (
...
from bigbytes.server.constants import DATA_PREP_SERVER_PORT
from bigbytes.server.docs_server import run_docs_server
from bigbytes.server.events.stream import EventStreamHandler
from bigbytes.server.file_observer import MetadataEventHandler
from bigbytes.server.kernel_output_parser import parse_output_message
from bigbytes.server.logger import Logger
from bigbytes.server.scheduler_manager import (
    SCHEDULER_AUTO_RESTART_INTERVAL,
    check_scheduler_status,
    scheduler_manager,
)
from bigbytes.server.setup import initialize_globals
...
from bigbytes.services.redis.redis import init_redis_client
from bigbytes.services.spark.models.applications import Application
from bigbytes.services.ssh.aws.emr.utils import file_path as file_path_aws_emr
from bigbytes.settings import (
    AUTHENTICATION_MODE,
    DEFAULT_OWNER_EMAIL,
...
from bigbytes.shared.constants import ENV_VAR_INSTANCE_TYPE, InstanceType
from bigbytes.shared.environments import is_debug, is_dev
from bigbytes.shared.io import chmod
from bigbytes.shared.logger import LoggingLevel, set_logging_format
from bigbytes.shared.singletons.memory import get_memory_manager_controller
from bigbytes.shared.utils import is_port_in_use
...
def replace_base_path(base_path: str) -> str:
    ...
    # get_variables_dir can return an s3 path. In that case, use the DEFAULT_BIGBYTES_DATA_DIR
    # directory.
    if directory.startswith('s3'):
        directory = os.path.join(os.path.expanduser(DEFAULT_BIGBYTES_DATA_DIR), get_repo_name())
...
def make_app(
    ...
    if ENABLE_PROMETHEUS:
        ...
        resource = Resource(attributes={SERVICE_NAME: 'bigbytes'})
...
async def main(
    ...
    if not status_only:
        from bigbytes.server.active_kernel import switch_active_kernel
        from bigbytes.server.kernels import DEFAULT_KERNEL_NAME
    ...
    logger.info(f'Bigbytes is running at {url} and serving project {project}')
    ...
    if preferences.sync_config:
        try:
            from bigbytes.data_preparation.sync import GitConfig
            from bigbytes.data_preparation.sync.git_sync import GitSync
    ...
    if not status_only:
        ...
        try:
            from bigbytes.services.ssh.aws.emr.models import create_tunnel
