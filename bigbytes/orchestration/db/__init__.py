...
from bigbytes.orchestration.constants import DATABASE_CONNECTION_URL_ENV_VAR
from bigbytes.orchestration.db.cache import CachingQuery, SessionWithCaching
from bigbytes.orchestration.db.setup import get_postgres_connection_url
from bigbytes.orchestration.db.utils import get_user_info_from_db_connection_url
from bigbytes.settings import OTEL_EXPORTER_OTLP_ENDPOINT
from bigbytes.settings.repo import get_variables_dir
from bigbytes.shared.environments import is_debug, is_test_bigbytes
...
if is_test_bigbytes():
    ...
    if pg_db_connection_url:
        ...
        if is_test_bigbytes():
            db_connection_url = f'sqlite:///{TEST_DB}'
        elif os.path.exists(os.path.join('bigbytes', 'orchestration', 'db')):
            # For local dev environment
            db_connection_url = 'sqlite:///bigbytes/orchestration/db/bigbytes.db'
        elif os.path.exists('bigbytes.db'):
            # For backward compatiblility
            db_connection_url = f'sqlite:///{get_variables_dir()}/bigbytes.db'
        else:
            # For new projects, create bigbytes.db in variables dir
            db_connection_url = f'sqlite:///{get_variables_dir()}/bigbytes.db'
        db_kwargs['connect_args']['check_same_thread'] = False
