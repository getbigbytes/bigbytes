...
from bigbytes import settings
from bigbytes.api.api_context import ApiContext
from bigbytes.api.errors import ApiError
from bigbytes.api.monitors.BaseMonitor import BaseMonitor
from bigbytes.api.operations.constants import (
    COOKIE_PREFIX,
    CREATE,
    DELETE,
    DETAIL,
...
from bigbytes.api.presenters.BasePresenter import CustomDict, CustomList
from bigbytes.api.resources.BaseResource import BaseResource
from bigbytes.api.result_set import ResultSet
...
from bigbytes.orchestration.db.errors import DoesNotExistError
from bigbytes.settings import REQUIRE_USER_PERMISSIONS
from bigbytes.shared.array import flatten
from bigbytes.shared.environments import is_debug
from bigbytes.shared.hash import ignore_keys, merge_dict
from bigbytes.shared.strings import classify
...
class BaseOperation:
        ...
        try:
            return getattr(
                importlib.import_module(
                    'bigbytes.api.monitors.{}Monitor'.format(self.__classified_class())
                ),
    ...
    def __parser_class(self) -> Union[BaseParser, None]:
        ...
        module_name = f'bigbytes.api.parsers.{klass}Parser'
    ...
    def __policy_class(self):
        return getattr(
            importlib.import_module(
                'bigbytes.api.policies.{}Policy'.format(self.__classified_class())
            ),
    ...
    def __presenter_class(self):
        return getattr(
            importlib.import_module(
                'bigbytes.api.presenters.{}Presenter'.format(self.__classified_class())
            ),
    ...
    def __resource_class(self):
        return getattr(
            importlib.import_module(
                'bigbytes.api.resources.{}Resource'.format(self.__classified_class())
            ),
    ...
    def __resource_parent_class(self):
        ...
        if entity_name:
            return getattr(
                importlib.import_module('bigbytes.api.resources.{}Resource'.format(entity_name)),
