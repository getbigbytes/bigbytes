...
from bigbytes.settings.models.configuration_option import (
    ConfigurationOption,
    ConfigurationType,
    OptionType,
)
...
class ConfigurationOptionTest(AsyncDBTestCase):
    ...
    async def test_fetch_project_platform_activated(self):
        with patch(
            'bigbytes.settings.models.configuration_option.project_platform_activated',
            lambda: True,
        ):
    ...
    async def test_fetch_with_resource_uuid(self):
        ...
        block = DBTBlock.create(
            block_type='dbt',
            configuration=dict(file_path='dir2/bigbytes.sql'),
            language='sql',
