...
class KafkaTests(TestCase):
    ...
    def test_init_client_with_topic(self):
        ...
        with patch('bigbytes.streaming.sources.kafka.KafkaConsumer') as mock_consumer:
        ...
        with patch.object(KafkaSource, 'init_client') as mock_init_client:
            source = KafkaSource(dict(
                ...
                serde_config=dict(
                    ...
                    schema_classpath='bigbytes.tests.base_test.TestCase',
                )
            ))
            mock_init_client.assert_called_once()
            self.assertEqual(source.config.serde_config.serialization_method, 'PROTOBUF')
            self.assertEqual(
                source.config.serde_config.schema_classpath, 'bigbytes.tests.base_test.TestCase')
