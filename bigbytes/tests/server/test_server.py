...
import bigbytes.server.server as server_module
from bigbytes.authentication.passwords import create_bcrypt_hash, generate_salt
from bigbytes.data_preparation.repo_manager import ProjectType, get_project_uuid
from bigbytes.orchestration.db.models.oauth import (
    Oauth2Application,
    Permission,
    Role,
    User,
...
from bigbytes.server.server import (
...
class ServerTests(AsyncDBTestCase):
    ...
    @patch('bigbytes.server.server.ROUTES_BASE_PATH', 'test_prefix')
    ...
    @patch('bigbytes.server.server.REQUESTS_BASE_PATH', 'random-test-string-1hasdfh')
    @patch('bigbytes.server.server.get_variables_dir')
    def test_replace_base_path(self, mock_variables_dir):
        mock_variables_dir.return_value = os.path.dirname(server_module.__file__)
        with patch(
            'bigbytes.server.server.BASE_PATH_EXPORTS_FOLDER',
            'base_path_test',
        ):
    ...
    @patch('bigbytes.server.server.REQUESTS_BASE_PATH', 'test_prefix')
    @patch('bigbytes.server.server.get_variables_dir')
    ...
    def test_replace_base_path_s3_directory(self, mock_variables_dir):
        ...
        with patch(
            'bigbytes.server.server.DEFAULT_BIGBYTES_DATA_DIR',
            test_dir,
        ):
            replace_base_path(base_path='test_prefix')
