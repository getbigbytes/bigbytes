...
@mock.patch('bigbytes.server.server.start_server')
class StartTests(TestCase):
    def tests_start_with_no_arguments(self, mock_start_sever):
        ...
        assert 'Bigbytes is running at http://' in result.output
    ...
    def test_start_with_host(self, mock_start_sever):
        ...
        assert result.exit_code == 0
        assert 'Bigbytes is running at http://127.0.0.1:6789' in result.output
    ...
    def test_start_with_port(self, mock_start_sever):
        ...
        assert result.exit_code == 0
        assert 'Bigbytes is running at http://localhost:8000' in result.output
