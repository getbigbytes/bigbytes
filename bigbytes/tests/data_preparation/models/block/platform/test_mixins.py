...
from bigbytes.data_preparation.models.block.dbt.block_sql import DBTBlock, DBTBlockSQL
...
class BlockWithProjectPlatformShared:
    def build_block(self, project_platform: bool = True, block_type: BlockType = None, **kwargs):
        with patch(
            ...
            with patch(
                ...
                with patch(
                    'bigbytes.settings.platform.project_platform_activated',
                    lambda: project_platform,
                ):
    ...
    def run_test_file_path(self, test_value: Callable):
        ...
        with patch('bigbytes.shared.path_fixer.add_absolute_path', lambda value, _=value: value):
            self.assertEqual(self.block.file_path, add_root_repo_path_to_relative_path(value))

        with patch('bigbytes.shared.path_fixer.add_absolute_path', lambda value, _: None):
...
class BlockWithProjectPlatformInactiveTest(BaseAPIEndpointTest, BlockWithProjectPlatformShared):
    ...
    def test_create(self):
        with patch('bigbytes.data_preparation.models.block.load_template') as mock_load_template:
            self.build_block(project_platform=False)
    ...
    def test_create_dbt(self):
        ...
        with patch(
            'bigbytes.data_preparation.models.block.File.from_path',
...
class BlockWithProjectPlatformActivatedTest(ProjectPlatformMixin, BlockWithProjectPlatformShared):
    ...
    def test_create(self):
        ...
        with patch('bigbytes.data_preparation.models.block.load_template') as mock_load_template:
            with patch(
                'bigbytes.data_preparation.models.block.from_another_project',
