...
from bigbytes.data_preparation.models.utils import infer_variable_type
...
from bigbytes.tests.factory import create_pipeline
from bigbytes.tests.test_shared import (
    build_iterable,
    build_list_complex,
    build_matrix_sparse,
    build_pandas,
...
class VariableTest(DBTestCase):
    ...
    def test_write_and_read_dataframe(self):
        with patch('bigbytes.data.models.manager.DataManager.writeable', return_value=False):
            with patch('bigbytes.data.models.manager.DataManager.readable', return_value=False):
    ...
    def test_write_statistics_for_polars_dataframe(self):
        with patch.multiple(
            'bigbytes.settings.server',
            MEMORY_MANAGER_PANDAS_V2=True,
            MEMORY_MANAGER_POLARS_V2=True,
            MEMORY_MANAGER_V2=True,
        ):
            with patch('bigbytes.data.models.manager.DataManager.writeable', return_value=True):
                data = build_polars()
                variable = Variable(
                    'var_polars',
                    self.pipeline.dir_path,
