...
from bigbytes.data_preparation.models.constants import PipelineStatus, PipelineType
...
class PipelineResourceTest(BaseApiTestCase):
    ...
    async def test_collection_without_cache(self):
        ...
        with patch.object(self.cache, 'get_model', lambda _x, repo_path=None: None):
            with patch.object(Pipeline, 'get_async', wraps=Pipeline.get_async) as mock_get_async:
                with patch(
                    'bigbytes.cache.pipeline.PipelineCache.initialize_cache',
                    __initialize_cache,
                ):
    ...
    async def test_collection_with_order_by(self):
        ...
        results = []
        with patch.object(self.cache, 'get_model', __get_model):
            with patch('bigbytes.cache.pipeline.PipelineCache.initialize_cache', __initialize_cache):
                results = await PipelineResource.collection({}, {}, None)
    ...
    async def test_get_model_all_projects(self):
        with patch(
            'bigbytes.api.resources.PipelineResource.project_platform_activated',
            lambda: True,
        ):
            with patch(
                'bigbytes.api.resources.PipelineResource.get_pipeline_from_platform_async',
            ) as mock:
