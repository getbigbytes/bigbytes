...
from bigbytes.authentication.ldap import LDAPConnection
from bigbytes.authentication.oauth2 import encode_token
from bigbytes.orchestration.db.models.oauth import Oauth2AccessToken, Role, User
...
class SessionOperationTests(BaseApiTestCase):
    ...
    async def test_execute_create_with_disable_notebook_edits(self):
        ...
        with patch('bigbytes.api.policies.BasePolicy.DISABLE_NOTEBOOK_EDIT_ACCESS', 1):
            with patch(
                'bigbytes.api.policies.BasePolicy.REQUIRE_USER_AUTHENTICATION', 1
            ):
                with patch(
                    'bigbytes.api.policies.BasePolicy.REQUIRE_USER_PERMISSIONS', 0
                ):
                    operation = self.build_operation(
                        action=constants.CREATE,
                        payload=dict(
    ...
    @patch('bigbytes.api.resources.SessionResource.AUTHENTICATION_MODE', 'ldap')
