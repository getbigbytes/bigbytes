...
from bigbytes.orchestration.db.models.base import BaseModel
...
from bigbytes.shared.strings import classify, singularize
...
def get_policy(resource: str):
    ...
    return getattr(
        importlib.import_module(f'bigbytes.api.policies.{model_name}Policy'),
        f'{model_name}Policy',
    )
...
def get_resource(resource: str):
    model_name = entity_name(resource)
    return getattr(
        importlib.import_module(f'bigbytes.api.resources.{model_name}Resource'),
    ...
    async def build_test_list_endpoint(
        ...
        with patch_manager(patch_function_settings or []) as mocks:
            with patch_object_manager(patch_object_settings or []) as mock_objects:
                with patch(
                    'bigbytes.api.policies.BasePolicy.REQUIRE_USER_AUTHENTICATION',
                    authentication or permissions or 0,
                ):
                    with patch(
                        'bigbytes.api.policies.BasePolicy.REQUIRE_USER_PERMISSIONS',
                        permissions or 0,
                    ):
                        meta = None
                        if build_meta:
