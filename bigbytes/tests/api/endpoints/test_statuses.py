...
class StatusWithProjectPlatformAPIEndpointTest(BaseAPIEndpointTest, ProjectPlatformMixin):
    def setUp(self):
        with patch('bigbytes.settings.platform.constants.project_platform_activated', lambda: True):
            with patch('bigbytes.settings.platform.project_platform_activated', lambda: True):
                with patch(
                    'bigbytes.settings.repo.project_platform_activated',
                    lambda: True,
                ):
                    super().setUp()
                    self.setup_final()
...
build_list_endpoint_tests(
    ...
    patch_function_settings=[
        ('bigbytes.settings.platform.constants.project_platform_activated', lambda: True),
