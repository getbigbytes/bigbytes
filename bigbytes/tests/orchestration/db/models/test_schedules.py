...
from bigbytes.tests.base_test import AsyncDBTestCase, DBTestCase
from bigbytes.tests.factory import (
    create_pipeline_run,
    create_pipeline_run_with_schedule,
    create_pipeline_with_blocks,
)
from bigbytes.tests.shared.mixins import ProjectPlatformMixin, build_hooks
...
class PipelineRunTests(DBTestCase):
    ...
    def test_executable_block_runs_with_data_integration_blocks(self):
        ...
        with patch(
            'bigbytes.data_preparation.models.project.Project.is_feature_enabled',
...
class PipelineScheduleProjectPlatformTests(ProjectPlatformMixin):
    ...
    def test_pipeline(self):
        with patch(
            'bigbytes.orchestration.db.models.schedules_project_platform.get_pipeline_from_platform',
        ) as mock:
...
class PipelineRunProjectPlatformTests(ProjectPlatformMixin, AsyncDBTestCase):
    ...
    async def test_logs_async(self):
        ...
        with patch(
            'bigbytes.orchestration.db.models.schedules_project_platform.'
            'get_pipeline_from_platform_async',
            __get_pipeline_from_platform_async,
        ):
            with patch(
                'bigbytes.orchestration.db.models.schedules_project_platform.LoggerManagerFactory',
    ...
    def test_get_variables(self):
        ...
        with patch(
            'bigbytes.orchestration.db.models.schedules_project_platform.get_global_variables',
            wraps=lambda pipeline_uuid, pipeline: {},
        ) as mock:
            with patch(
                'bigbytes.data_preparation.models.pipeline.project_platform_activated',
                lambda: True,
            ):
