...
const PIPELINE_DYNAMIC: PipelineType = {
  ...
  type: 'python',
  uuid: 'dynamic',
  variables_dir: '/root/.bigbytes_data/memory_upgrade_v2',
  spark_config: {},
  blocks: [
    {
...
const PIPELINE_DEPLOY = {
  ...
  variables_dir: '/root/.bigbytes_data/unit_3_observability',
  variables: {
    destroy: 1,
    password: 'password',
  ...
  blocks: [
    ...
    },
    {
      ...
      executor_type: 'local_python',
      has_callback: false,
      name: 'Create IAM policy to deploy Bigbytes',
      language: 'markdown',
      retry_config: null,
      status: 'updated',
      timeout: null,
      ...
      conditional_blocks: [],
      content:
        '## Setup AWS permissions for deploying\n\nRun the next block to set all of this up automtically.\n\n### Create IAM policy to deploy Bigbytes\n\n1. Add these\n   [permissions](https://docs.bigbytes.io/production/deploying-to-cloud/aws/terraform-apply-policy)\n   using the JSON editor.\n\n1. Policy name: **TerraformApplyDeployBigbytes**\n\n> In order to perform the above action, you need the following permissions:\n\n```json\niam:CreatePolicy\n```\n\n```sh\naws iam create-policy --policy-name TerraformApplyDeployBigbytes --policy-document \\\n  "$(curl -s https://raw.githubusercontent.com/digitranslab/bigbytes-terraform-templates/master/aws/policies/TerraformApplyDeployBigbytes.json)"\n```\n\n<br />\n\n---\n\n<br />\n\n### Create AWS policies to delete resources\n\n1. Add these\n   [permissions](https://docs.bigbytes.io/production/deploying-to-cloud/aws/terraform-destroy-policy)\n   using the JSON editor.\n\n1. Policy name: **TerraformDestroyDeleteResources**\n\n> In order to perform the above action, you need the following permissions:\n\n```json\niam:CreatePolicy\n```\n\n```sh\naws iam create-policy --policy-name TerraformDestroyDeleteResources --policy-document \\\n  "$(curl -s https://raw.githubusercontent.com/digitranslab/bigbytes-terraform-templates/master/aws/policies/TerraformDestroyDeleteResources.json)"\n```\n\n<br />\n\n---\n\n<br />\n\n### Create IAM user\n\n1. User name: **BigbytesDeployer**\n\n1. Attach policies directly:\n\n- **TerraformApplyDeployBigbytes**\n- **TerraformDestroyDeleteResources**\n\n1. After creating the user, create an access key and secret key.\n\n1. Use case: **Command Line Interface (CLI)**\n\n##### Create IAM User\n\n**Creating the IAM User (BigbytesDeployer):**\n\n> In order to perform the above action, you need the following permissions:\n\n```json\niam:CreateUser\n```\n\n```sh\naws iam create-user --user-name BigbytesDeployer\n```\n\n##### Attach Policies to User\n\n> In order to perform the above action, you need the following permissions:\n\n```json\niam:ListPolicies\n```\n\n**Attach TerraformApplyDeployBigbytes Policy:**\n\n_First, find the Policy ARN by listing the policies.\n\\_Then, attach the policy to the user using the ARN obtained:_\n\n```sh\naws iam attach-user-policy \\\n  --policy-arn $(aws iam list-policies \\\n  --query "Policies[?PolicyName==\\`TerraformApplyDeployBigbytes\\`].Arn" \\\n  --output text) \\\n  --user-name BigbytesDeployer\n```\n\n**Attach TerraformDestroyDeleteResources Policy:**\n\n_Find the Policy ARN. \\_Attach the policy to the user:_\n\n```sh\naws iam attach-user-policy \\\n  --policy-arn $(aws iam list-policies \\\n  --query "Policies[?PolicyName==\\`TerraformDestroyDeleteResources\\`].Arn" \\\n  --output text) \\\n  --user-name BigbytesDeployer\n```\n\n##### Create Access Key for the User\n\n```sh\naws iam create-access-key \\\n  --user-name BigbytesDeployer \\\n  --output json | jq -r \'"[bigbytes-deployer]\\naws_access_key_id = \\(.AccessKey.AccessKeyId)\\naws_secret_access_key = \\(.AccessKey.SecretAccessKey)"\' >> ~/.aws/credentials\nexport AWS_PROFILE="bigbytes-deployer"\n```\n',
      outputs: [],
      metadata: {},
      tags: [],
      pipelines: [
    ...
    },
    {
      ...
      conditional_blocks: [],
      content:
        '## Terraform setup\n\n> If you cloned Bigbytes’s [MLOps repository](https://github.com/digitranslab/mlops), \n> skip this section and go to the **Customize Terraform configurations** section.\n\n\nInstall and setup Terraform on your local machine.\nFollow the guide here or the detailed guide in the \n[Bigbytes arcane library aka developer documentation](https://docs.bigbytes.io/production/deploying-to-cloud/using-terraform).\n\n<br />\n\n---\n\n<br />\n\n\n### Customize Terraform configurations\n\nOpen the file `bigbytes-ops/mlops/terraform/aws/variables.tf` and update the following variables:\n\n```hcl\nvariable "docker_image" {\n  description = "Docker image url used in ECS task."\n  default     = "bigbytes/bigbytes:alpha"\n}\n\nvariable "app_name" {\n  type        = string\n  description = "Application Name"\n  default     = "mlops"\n}\n\nvariable "aws_region" {\n  type        = string\n  description = "AWS Region"\n  default     = "us-west-2"\n}\n\nvariable "availability_zones" {\n  description = "List of availability zones"\n  default     = ["us-west-2a", "us-west-2b"]\n}\n```\n\nOpen the file `bigbytes-ops/mlops/terraform/aws/env_vars.json` and update 1 variable\nif your project name isn’t `mlops`:\n\n```json\n[\n  {\n    "name": "BIGBYTES_PRESENTERS_DIRECTORY",\n    "value": "[change to your project name]/presenters"\n  }\n]\n```\n\n<br />\n\n---\n\n<br />\n\n#### Run the following commands to deploy the application to AWS:\n\nRun the command after changing directories into: `bigbytes-ops/mlops/terraform/aws`\n\n```bash\nterraform init\nterraform apply\n```\n\nHere are detailed instructions on how to\n[deploy Bigbytes to AWS using Terraform](https://docs.bigbytes.io/production/deploying-to-cloud/aws/setup).',
      outputs: [],
      metadata: {},
      tags: [],
      pipelines: [
...
export const PIPELINE_RAG: PipelineType = {
  ...
  uuid: 'Bigbytesr data planeswalker',
  blocks: [
    {
