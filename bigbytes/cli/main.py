...
from bigbytes.cli.utils import parse_runtime_variables
from bigbytes.data_preparation.repo_manager import ProjectType
from bigbytes.services.newrelic import initialize_new_relic
...
INIT_PROJECT_PATH_DEFAULT = typer.Argument(..., help='path of the Bigbytes project to be created.')
...
START_PROJECT_PATH_DEFAULT = typer.Argument(
    os.getcwd(), help='path of the Bigbytes project to be loaded.'
)
...
RUN_PROJECT_PATH_DEFAULT = typer.Argument(
    ..., help='path of the Bigbytes project that contains the pipeline.'
)
...
CLEAN_LOGS_PROJECT_PATH_DEFAULT = typer.Argument(
    ..., help='path of the Bigbytes project to clean old logs.'
)
...
CLEAN_VARIABLES_PROJECT_PATH_DEFAULT = typer.Argument(
    ..., help='path of the Bigbytes project to clean variables.'
)
...
CREATE_SPARK_CLUSTER_PROJECT_PATH_DEFAULT = typer.Argument(
    ..., help='path of the Bigbytes project that contains the EMR config.'
)
...
def init(
    ...
    """
    Initialize Bigbytes project.
    """
    from bigbytes.data_preparation.repo_manager import init_repo
    ...
    print(f'Initialized Bigbytes project at {repo_path}')
...
def start(
    ...
    from bigbytes.server.server import start_server
...
def run(
    ...
    from bigbytes.data_preparation.executors.executor_factory import ExecutorFactory
    from bigbytes.data_preparation.models.pipeline import Pipeline
    from bigbytes.data_preparation.sync.git_sync import get_sync_config
    ...
    from bigbytes.orchestration.utils.git import log_git_sync, run_git_sync
    from bigbytes.server.logger import Logger
    from bigbytes.settings import SENTRY_DSN, SENTRY_TRACES_SAMPLE_RATE
    ...
    with (
        newrelic.agent.BackgroundTask(application, name='bigbytes-run', group='Task')
        if enable_new_relic
        else nullcontext()
    ):
...
def clean_cached_variables(
    ...
    from bigbytes.data_preparation.variable_manager import clean_variables
...
def clean_old_logs(
    ...
    from bigbytes.data_preparation.logging.logger_manager_factory import (
...
def create_spark_cluster(
    ...
    """
    Create EMR cluster for Bigbytes project.
    """
    from bigbytes.services.aws.emr.launcher import create_cluster
