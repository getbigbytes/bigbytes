...
from bigbytes.data_preparation.executors.pipeline_executor import PipelineExecutor
...
from bigbytes.shared.code import is_pyspark_code
...
class ExecutorFactory:
    ...
    def get_pipeline_executor(
        ...
        if executor_type == ExecutorType.PYSPARK:
            from bigbytes.data_preparation.executors.pyspark_pipeline_executor import (
            ...
            from bigbytes.data_preparation.executors.ecs_pipeline_executor import (
                EcsPipelineExecutor,
            )
            return EcsPipelineExecutor(pipeline, execution_partition=execution_partition)
        elif executor_type == ExecutorType.K8S:
            from bigbytes.data_preparation.executors.k8s_pipeline_executor import (
                K8sPipelineExecutor,
            )
    ...
    def get_block_executor(
        ...
        if executor_type == ExecutorType.PYSPARK:
            from bigbytes.data_preparation.executors.pyspark_block_executor import (
                PySparkBlockExecutor,
            )
            return PySparkBlockExecutor(**executor_kwargs)
        elif executor_type == ExecutorType.AZURE_CONTAINER_INSTANCE:
            from bigbytes.data_preparation.executors.azure_container_instance_executor import (
                AzureContainerInstanceExecutor,
            )
            return AzureContainerInstanceExecutor(**executor_kwargs)
        elif executor_type == ExecutorType.ECS:
            from bigbytes.data_preparation.executors.ecs_block_executor import (
                EcsBlockExecutor,
            )
            return EcsBlockExecutor(**executor_kwargs)
        elif executor_type == ExecutorType.GCP_CLOUD_RUN:
            from bigbytes.data_preparation.executors.gcp_cloud_run_block_executor import (
                GcpCloudRunBlockExecutor,
            )
            return GcpCloudRunBlockExecutor(**executor_kwargs)
        elif executor_type == ExecutorType.K8S:
            from bigbytes.data_preparation.executors.k8s_block_executor import (
                K8sBlockExecutor,
            )
