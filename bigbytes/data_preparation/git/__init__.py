...
from bigbytes.data_preparation.git.constants import DEFAULT_KNOWN_HOSTS_FILE
...
from bigbytes.settings.platform import git_settings, project_platform_activated
    ...
    def remotes(self, limit: int = 40, user: User = None) -> List[Dict]:
        ...
        for idx, remote in enumerate(self.repo.remotes):
            ...
            try:
                ...
                print('WARNING (bigbytes.data_preparation.git.remotes):')
                print(err)

            try:
                remote_refs = remote.refs
                if len(remote_refs) == 0 and user:
                    from bigbytes.data_preparation.git import api
