...
class Client(ABC):
    ...
    def get_client_for_provider(cls, provider: str):
        if not provider or provider == ProviderName.GITHUB:
            from bigbytes.data_preparation.git.clients.github import GitHubClient

            return GitHubClient
        elif provider == ProviderName.BITBUCKET:
            from bigbytes.data_preparation.git.clients.bitbucket import BitbucketClient

            return BitbucketClient
        elif provider == ProviderName.GITLAB:
            from bigbytes.data_preparation.git.clients.gitlab import GitlabClient

            return GitlabClient
        elif provider == ProviderName.GHE:
            from bigbytes.data_preparation.git.clients.ghe import GHEClient

            return GHEClient
        elif provider == ProviderName.AZURE_DEVOPS:
            from bigbytes.data_preparation.git.clients.azure_devops import (
                AzureDevopsClient,
            )
