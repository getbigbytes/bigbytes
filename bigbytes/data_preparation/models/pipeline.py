...
from bigbytes.cache.mem_lru import read_yaml_file, read_yaml_file_async
...
from bigbytes.data_preparation.models.block import Block, run_blocks, run_blocks_sync
...
from bigbytes.data_preparation.models.block.extension.utils import compare_extension
...
from bigbytes.data_preparation.models.errors import (
    FileWriteError,
...
from bigbytes.data_preparation.models.pipelines.models import PipelineSettings
...
from bigbytes.data_preparation.models.utils import (
...
from bigbytes.data_preparation.shared.secrets import (
    delete_secrets_dir,
    rename_pipeline_secrets_dir,
)
from bigbytes.data_preparation.shared.utils import get_template_vars
from bigbytes.data_preparation.templates.utils import copy_template_directory
...
from bigbytes.orchestration.notification.config import NotificationConfig
from bigbytes.orchestration.notification.sender import NotificationSender
from bigbytes.settings.platform import build_repo_path_for_all_projects
...
from bigbytes.shared.hash import extract, ignore_keys, index_by, merge_dict
from bigbytes.shared.io import safe_write, safe_write_async
...
from bigbytes.shared.yaml import load_yaml, yaml
...
class Pipeline:
    ...
    def build_config_path(
        ...
        if project_platform_activated() and not use_repo_path:
            from bigbytes.settings.platform.utils import get_pipeline_config_path
    ...
    def execute_sync(
        ...
        """
        if self.type == PipelineType.STREAMING:
            from bigbytes.data_preparation.executors.streaming_pipeline_executor import (
    ...
    def __initialize_blocks_by_uuid(
        ...
        if execution_framework is not None and \
           ExecutionFrameworkUUID.has_value(execution_framework):
            ...
            from bigbytes.frameworks.execution.constants import (
    ...
    async def update(self, data, update_content=False):
        from bigbytes.orchestration.db.models.utils import (
        ...
        if update_content:
            ...
            if len(arr) >= 1:
                if Project.is_feature_enabled_in_root_or_active_project(FeatureUUID.GLOBAL_HOOKS):
                    from bigbytes.data_preparation.models.global_hooks.models import (
                        GlobalHooks,
                    )
            ...
            for tup in arr:
                ...
                for block_data in blocks_arr:
                    ...
                    if 'content' in block_data:
                        from bigbytes.cache.block_action_object import (
        ...
        if any(
            ...
            from bigbytes.data_preparation.models.block.dbt import DBTBlock
