...
from bigbytes.data_integrations.logger.utils import print_logs_from_output
from bigbytes.data_integrations.utils.config import (
    build_config,
    get_catalog,
...
class IntegrationPipeline(Pipeline):
    ...
    def destination(self) -> Any:
        if self.destination_uuid:
            return importlib.import_module(
                f'bigbytes_integrations.destinations.{self.destination_uuid}',
            )
    ...
    def source(self) -> Any:
        if self.source_uuid:
            return importlib.import_module(f'bigbytes_integrations.sources.{self.source_uuid}')
    ...
    def transformer_file_path(self) -> str:
        transformer_file = importlib.import_module('bigbytes_integrations.transformers.base')
